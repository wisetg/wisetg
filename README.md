<p align="right">
  <a href="#projects-ive-worked-on-english">🌐 English Version</a> | 
  <a href="#내가-참여한-프로젝트-한국어">🌐 한국어 버전</a> | 
  <a href="#参画プロジェクト-日本語">🌐 日本語バージョン</a>
</p>

---

# Projects I've Worked On (English)

## TEAM RUNWAY (Team Project)

<details>
<summary><b><span style="color: #1f4d2b;">[Project-Unbound]</span> (Titanomachia)</b></summary>
<blockquote>
<br>

**Korea Aerospace University Software Engineering 6-Player Collaborative Project** A 2D Metroidvania action game based on Greek and Roman mythology, developed with the goal of submitting to the campus game contest.

#### Tech Stacks
![Unity](https://img.shields.io/badge/Unity-2022.3+-black?style=flat-square&logo=Unity&logoColor=white) ![C#](https://img.shields.io/badge/C%23-%23239120?style=flat-square&logo=c-sharp&logoColor=white)

#### My Roles & Responsibilities
| Category | Details |
| :--- | :--- |
| **Planning & Design** | Main game planning, storyline structuring, level design, and map asset creation |
| **Development** | UI system, game map implementation, and object interaction logic development |

#### Links
- [Download Build File (Google Drive)](https://drive.google.com/file/d/15Y4yAB_2_XdKhQOHlKSqdXLDX4dgoEs0/view?usp=sharing)
- *Note: This repository is managed as Private due to the organization's security policy.*

<br>
</blockquote>
</details>

<details>
<summary><b><span style="color: #1f4d2b;">[Project-SilverBullet]</span></b></summary>
<blockquote>
<br>

**Run & Gun Platformer Game** A unique evasion-focused platformer action game where players strategically utilize bullets fired by enemies.

#### Tech Stacks
![Unity](https://img.shields.io/badge/Unity-2022.3+-black?style=flat-square&logo=Unity&logoColor=white) ![C#](https://img.shields.io/badge/C%23-%23239120?style=flat-square&logo=c-sharp&logoColor=white) ![Aseprite](https://img.shields.io/badge/Aseprite-%238D52D5?style=flat-square&logo=Aseprite&logoColor=white)

#### My Roles & Responsibilities
- **World Build:** Formulated the overall game storyline and lore
- **Level Design:** Designed core playable areas and customized map layouts
- **Art:** Designed map backgrounds and assets using pixel art

#### Links
- *Now Making...*
- *Note: This repository is managed as Private due to the organization's security policy.*

<br>
</blockquote>
</details>

## My Own Project

<details>
<summary><b><span style="color: #1f4d2b;">[Project-Far_Side_of_the_Moon]</span></b></summary>
<blockquote>
<br>

**Story-driven Platformer with Fighting Mechanism** A solo project that combines platformer terrain navigation with the precise command controls of fighting games to deliver a dynamic narrative and engaging mechanics.

#### Tech Stacks
![Unity](https://img.shields.io/badge/Unity-2022.3+-black?style=flat-square&logo=Unity&logoColor=white) ![C#](https://img.shields.io/badge/C%23-%23239120?style=flat-square&logo=c-sharp&logoColor=white) ![Aseprite](https://img.shields.io/badge/Aseprite-%238D52D5?style=flat-square&logo=Aseprite&logoColor=white)

#### My Roles & Responsibilities
- **Solo Project:** Overseeing the entirety of planning, scenario writing, map level design, art asset drawing, and client feature development.

#### Links
- *Now Making...*
- *Note: This repository is managed as Private for security and intellectual property protection.*

<br>
</blockquote>
</details>

## Other Projects

<details>
<summary><b><span style="color: #1f4d2b;">[cn-chat]</span></b></summary>
<blockquote>
<br>

A real-time chatting application consisting of a Django-based backend and a React-based frontend.

#### Tech Stacks
- Backend: Django, Django REST Framework, Django Channels (WebSocket)
- Frontend: React
- Database: SQLite (For local development)
- Authentication: JWT Token-based authentication

#### My Roles & Responsibilities
- Implemented file transfer features excluding images (documents/audio file transfer development)

#### Links
- https://github.com/psw204/cn-chat

<br>
</blockquote>
</details>

<details>
<summary><b><span style="color: #1f4d2b;">[Object_Oriented_Programming]</span></b></summary>
<blockquote>
<br>

Reptile Growth Diary Application "My Rep" Development Project

#### Tech Stacks
- Android Studio, C++

#### My Roles & Responsibilities
- Frontend & backend, mainpage and diarypage development

#### Links
- https://github.com/Ophelia0419/Object_Oriented_Programming

<br>
</blockquote>
</details>

<br>
<br>

---

# 내가 참여한 프로젝트 (한국어)

## TEAM RUNWAY (팀 프로젝트)

<details>
<summary><b><span style="color: #1f4d2b;">[Project-Unbound]</span> (Titanomachia)</b></summary>
<blockquote>
<br>

**항공대 소프트웨어학과 6인 협업 프로젝트** 교내 게임공모전 출품을 목표로 제작된 그리스 로마 신화 기반의 2D 메트로베니아 액션 게임입니다.

#### 기술 스택
![Unity](https://img.shields.io/badge/Unity-2022.3+-black?style=flat-square&logo=Unity&logoColor=white) ![C#](https://img.shields.io/badge/C%23-%23239120?style=flat-square&logo=c-sharp&logoColor=white)

#### 역할
| 분류 | 상세 내용 |
| :--- | :--- |
| **기획 & 디자인** | 게임 메인 기획, 스토리 라인 구축, 레벨 디자인 및 맵 에셋 제작 |
| **개발 (Dev)** | UI 시스템, 게임 맵 구현 및 오브젝트 상호작용 로직 개발 |

#### 링크
- [빌드 파일 다운로드 (Google Drive)](https://drive.google.com/file/d/15Y4yAB_2_XdKhQOHlKSqdXLDX4dgoEs0/view?usp=sharing)
- *본 저장소는 소속 조직의 보안 정책으로 인해 비공개(Private)로 관리되고 있습니다.*

<br>
</blockquote>
</details>

<details>
<summary><b><span style="color: #1f4d2b;">[Project-SilverBullet]</span></b></summary>
<blockquote>
<br>

**Run & Gun Platformer Game** 적이 발사한 총알을 역으로 이용하는 독특한 회피 기동 중심의 플랫포머 액션 게임입니다.

#### 기술 스택
![Unity](https://img.shields.io/badge/Unity-2022.3+-black?style=flat-square&logo=Unity&logoColor=white) ![C#](https://img.shields.io/badge/C%23-%23239120?style=flat-square&logo=c-sharp&logoColor=white) ![Aseprite](https://img.shields.io/badge/Aseprite-%238D52D5?style=flat-square&logo=Aseprite&logoColor=white)

#### 역할
- **World Build:** 전체 게임 스토리 라인 구상
- **Level Design:** 핵심 플레이 구역 레벨 디자인 및 전용 맵 구성
- **Art:** 픽셀 아트를 활용한 맵 배경 및 에셋 디자인

#### 링크
- *Now Making...*
- *본 저장소는 소속 조직의 보안 정책으로 인해 비공개(Private)로 관리되고 있습니다.*

<br>
</blockquote>
</details>

## 개인 프로젝트

<details>
<summary><b><span style="color: #1f4d2b;">[Project-Far_Side_of_the_Moon]</span></b></summary>
<blockquote>
<br>

**Story-driven Platformer with Fighting Mechanism** 플랫포머의 지형지물 이동과 대전 격투 게임의 정교한 커맨드 조작을 결합하여, 다채로운 손맛과 역동적인 서사를 전달하는 게임입니다.

#### 기술 스택
![Unity](https://img.shields.io/badge/Unity-2022.3+-black?style=flat-square&logo=Unity&logoColor=white) ![C#](https://img.shields.io/badge/C%23-%23239120?style=flat-square&logo=c-sharp&logoColor=white) ![Aseprite](https://img.shields.io/badge/Aseprite-%238D52D5?style=flat-square&logo=Aseprite&logoColor=white)

#### 역할
- **1인 전담 개발 (Solo Project):** 기획, 시나리오, 맵 레벨 디자인, 아트 에셋 드로잉, 클라이언트 기능 개발 전반 총괄

#### 링크
- *Now Making...*
- *본 저장소는 보안 및 IP 보호를 위해 비공개(Private)로 관리되고 있습니다.*

<br>
</blockquote>
</details>

## 기타 프로젝트

<details>
<summary><b><span style="color: #1f4d2b;">[cn-chat]</span></b></summary>
<blockquote>
<br>

Django 기반 백엔드와 React 기반 프론트엔드로 구성된 실시간 채팅 애플리케이션

#### 기술 스택
- 백엔드: Django, Django REST Framework, Django Channels (WebSocket)
- 프론트엔드: React
- 데이터베이스: SQLite (로컬 개발용)
- 인증 시스템: JWT 토큰 기반 인증

#### 역할
- 이미지 외 파일 전송 기능 구현 (문서/오디오 전송)

#### 링크
- https://github.com/psw204/cn-chat

<br>
</blockquote>
</details>

<details>
<summary><b><span style="color: #1f4d2b;">[Object_Oriented_Programming]</span></b></summary>
<blockquote>
<br>

파충류 성장 기록 앱 "My Rep" 개발 프로젝트

#### 기술 스택
- Android Studio, C++

#### 역할
- Frontend & backend, mainpage and diarypage

#### 링크
- https://github.com/Ophelia0419/Object_Oriented_Programming

<br>
</blockquote>
</details>

<br>
<br>

---

# 参画プロジェクト (日本語)

## TEAM RUNWAY (チームプロジェクト)

<details>
<summary><b><span style="color: #1f4d2b;">[Project-Unbound]</span> (Titanomachia)</b></summary>
<blockquote>
<br>

**韓国航空大学校 ソフトウェア工学科 6人共同開発プロジェクト** 校内ゲームコンテストへの出展を目標に制作された、ギリシャ・ローマ神話をモチーフにした2Dメトロイドヴァニア系アクションゲームです。

#### 開発スタック
![Unity](https://img.shields.io/badge/Unity-2022.3+-black?style=flat-square&logo=Unity&logoColor=white) ![C#](https://img.shields.io/badge/C%23-%23239120?style=flat-square&logo=c-sharp&logoColor=white)

#### 担当領域
| 区分 | 業務詳細 |
| :--- | :--- |
| **企画・デザイン** | ゲームメイン企画、シナリオ構築、レベルデザイン、およびマップアセット制作 |
| **開発 (Dev)** | UIシステム、ゲームマップ実装、およびオブジェクト相互作用ロジック開発 |

#### リンク
- [ビルドファイルのダウンロード (Google Drive)](https://drive.google.com/file/d/15Y4yAB_2_XdKhQOHlKSqdXLDX4dgoEs0/view?usp=sharing)
- *※本リポジトリは所属組織のセキュリティポリシーのため、非公開(Private)として管理されています。*

<br>
</blockquote>
</details>

<details>
<summary><b><span style="color: #1f4d2b;">[Project-SilverBullet]</span></b></summary>
<blockquote>
<br>

**Run & Gun Platformer Game** 敵が放った弾丸を逆手に取り、独自の回避機動を主力として展開する、ユニークなプラットフォーマーアクションゲームです。

#### 開発スタック
![Unity](https://img.shields.io/badge/Unity-2022.3+-black?style=flat-square&logo=Unity&logoColor=white) ![C#](https://img.shields.io/badge/C%23-%23239120?style=flat-square&logo=c-sharp&logoColor=white) ![Aseprite](https://img.shields.io/badge/Aseprite-%238D52D5?style=flat-square&logo=Aseprite&logoColor=white)

#### 担当領域
- **World Build:** ゲーム全体のストーリーおよび世界観の構築
- **Level Design:** コアプレイエリアのレベルデザインおよび専用マップ構成
- **Art:** ピクセルアートを用いたマップ背景およびアセットデザイン

#### リンク
- *Now Making... (開発進行中)*
- *※本リポジトリは所属組織のセキュリティポリシーのため、非公開(Private)として管理されています。*

<br>
</blockquote>
</details>

## 個人プロジェクト

<details>
<summary><b><span style="color: #1f4d2b;">[Project-Far_Side_of_the_Moon]</span></b></summary>
<blockquote>
<br>

**Story-driven Platformer with Fighting Mechanism** プラットフォーマーによる地形移動と、格闘ゲームの精密なコマンド入力を融合させることで、多彩な操作感とダイナミックな物語展開を提供する個人プロジェクトです。

#### 開発スタック
![Unity](https://img.shields.io/badge/Unity-2022.3+-black?style=flat-square&logo=Unity&logoColor=white) ![C#](https://img.shields.io/badge/C%23-%23239120?style=flat-square&logo=c-sharp&logoColor=white) ![Aseprite](https://img.shields.io/badge/Aseprite-%238D52D5?style=flat-square&logo=Aseprite&logoColor=white)

#### 担当領域
- **個人開発 (Solo Project):** 企画、シナリオ、マップレベルデザイン、ドット絵アセット制作、クライアント機能開発全般を単独で統括

#### リンク
- *Now Making... (開発進行中)*
- *※本リポジトリは、セキュリティおよび知的財産権保護のため、非公開(Private)として管理されています。*

<br>
</blockquote>
</details>

## その他のプロジェクト

<details>
<summary><b><span style="color: #1f4d2b;">[cn-chat]</span></b></summary>
<blockquote>
<br>

DjangoベースのバックエンドとReactベースのフロントエンドで構成されたリアルタイムチャットアプリケーションです。

#### 開発スタック
- バックエンド: Django, Django REST Framework, Django Channels (WebSocket)
- フロントエンド: React
- データベース: SQLite (ローカル開発用)
- 認証システム: JWTトークンベース認証

#### 担当領域
- 画像以外のファイル転送機能の実装 (文書/オーディオ転送機能開発)

#### リンク
- https://github.com/psw204/cn-chat

<br>
</blockquote>
</details>

<details>
<summary><b><span style="color: #1f4d2b;">[Object_Oriented_Programming]</span></b></summary>
<blockquote>
<br>

爬虫類成長記録アプリ「My Rep」開発プロジェクト

#### 開発スタック
- Android Studio, C++

#### 担当領域
- フロントエンド・バックエンド、メインページおよびダイアリーページの開発

#### リンク
- https://github.com/Ophelia0419/Object_Oriented_Programming

<br>
</blockquote>
</details>
